...
1. **Authentication Issue**: I had trouble fetching the public IP address and public hostname of the EC2 instance. I discovered it was an authentication issue while reading this documentation and figured out that I needed to use tokens.

2. **Attempt to Secure PHP Script with Environment Variables**: I tried to secure the PHP script by using environment variables to store the database credentials. I did a quick research, read some documentation and found a package called `PHP dotenv` that could help me achieve this. I installed the package using `composer` and created a `.env` file to store the database credentials as environment variables. I also updated the PHP script to use the `dotenv` package to load the database credentials from the `.env` file. The script connected to the MySQL database and retrieved the to-do list items from the `todo_list` table. The script was secured as the database credentials were not exposed in the script.

```

## Tasks

### MySQL syntax and common commands: side Self-study
...
- Verify nano and vim are installed on the EC2 instance by running the following commands: `vi --version` and `nano --version`.
- Update inbound rules for the EC2 instance to allow HTTP and HTTPS traffic.

#### My AWS EC2 instance after connecting to it using SSH

![screenshot of EC2 instance after SSH into it](images/Lemp-ec2-ubuntu-server.png)

#### My nano and vim version on the EC2 instance

##### Vim text editor version

![Screenshot of the vim text editor version on the EC2 instance](images/vim-text-editor-version.png)

##### Nano text editor version

![Screenshot of the vim text editor version on the EC2 instance](images/nano-text-editor.png)
...
![Status of Nginx webserver on the AWS EC2 instance](images/nginx-status.png)
...
- Remove the test database and any access to it
- Validate user password strength by default.
- remove anonymous users
I made these adjustments to secure the MySQL installation on the EC2 instance.

#### MySQL Installation Status

![mySQL server status indicating an actively running Mysql server](images/mysql-server-status.png)
...
![terminal output showing PHP version](images/php-version-status.png)
...
### 105 Adding a New NGINX Virtual Host
...
![syntax check result showing zero syntax errors in the config file for the new NGINX virtual host](images/nginx-config-syntax-check-result.png)
...
![browser output showing the expected HTML page](images/Working-index-page.png)
...
![Image showing the PHP info page](nginx-php-page.png)
...
### 107 Retrieve Data from MySQL Database
...
![termninal output showing connected user and example_database](images/connected-mysql-user.png)
...
![Todo Lists added and retrieved from the example_database](images/todo_lists.png)
...
#### Create a PHP script to retrieve data from the MySQL database
...
I had to install [PHP composer](https://www.digitalocean.com/community/tutorials/what-is-composer) to use the `dotenv` package. I installed the `dotenv` package using the following command:
...
I will create a `.env` file in the `/var/www/projectLEMP/` directory to store the database credentials as environment variables.
...
![page showing all the data retrieved from the MySQL page using the PHP script ](images/todo-list-php-page.png)
